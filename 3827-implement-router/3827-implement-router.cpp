...
public:
    ...
    bool addPacket(int source, int destination, int timestamp) {
        string tempHash = to_string(source) + "-" + to_string(destination) +
                          "-" + to_string(timestamp);

        if (packetSet.count(tempHash))
            return false;

        if (queue.size() >= memoryLimit) {
            vector<int> temp = queue.front();
            queue.pop();
            string canHash = to_string(temp[0]) + "-" + to_string(temp[1]) +
                             "-" + to_string(temp[2]);
            packetSet.erase(canHash);
            destinationTimestamps[temp[1]].erase(
                destinationTimestamps[temp[1]].begin());
        }
    ...
    vector<int> forwardPacket() {
        ...
        queue.pop();
        string canHash = to_string(packet[0]) + "-" + to_string(packet[1]) +
                         "-" + to_string(packet[2]);
        packetSet.erase(canHash);
        destinationTimestamps[packet[1]].erase(
            destinationTimestamps[packet[1]].begin());
